...
async def show_version_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "✅ Тест «Индекс потери себя» с подбором трека\n"
        "✅ Персональный трек под твой результат\n\n"
        "💎 Рабочая тетрадь в PDF\n"
        "Один файл на все 5 дней с полями для записей.\n"
        "Структурирует каждый день, помогает закрепить инсайты и не потерять важное.\n\n"
        "💎 Бонусное голосовое на 6-й день\n"
        "«Как закрепить результат и не откатиться назад»\n"
        "Тёплый аудио-разбор для мягкого завершения челленджа.\n\n"
        "💎 Закрытый чат с участницами твоего потока\n"
        "Общее пространство для поддержки, обмена инсайтами и чувства «мы вместе».\n"
        "Даже если в потоке 5 человек — эффект группы работает.\n\n"
        "💎 Персональная аудио-рефлексия от меня на 7-й день\n"
        "Ты присылаешь голосом свой главный инсайт\n"
        "Я отвечаю 1–2 минутным голосовым персонально для тебя\n"
        "Лимит — до 30 участниц в потоке\n\n"
        "✨ Всё это сделано для того, чтобы дать больше глубины, поддержки и структуры.\n"
        "Бесплатная версия остаётся — выбор за тобой.\n\n"
...
async def send_final_invitation(chat_id):
    ...
    text = (
        ...
        "Напиши мне слово «РАЗВОРОТ» и узнай детали 🌸"
    )
