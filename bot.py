...
DB_COLUMNS = {
    ...
    "mini_done_days": "TEXT DEFAULT '[]'",
    "reflection_choices": "TEXT DEFAULT '[]'",
    "hard_help_count": "INTEGER DEFAULT 0",
}
...
# ============================================================
# НОВЫЕ ФОРМАТЫ ЧЕЛЛЕНДЖА
# ============================================================

MINI_TASKS = {
    1: "Прямо сейчас спроси себя: «Что я хочу сегодня?» Не «что надо», а именно «чего хочу я». Ответь одним словом.",
    2: "Потренируй паузу: если сегодня от тебя что-то попросят, не отвечай автоматически. Скажи: «Мне нужно подумать, я отвечу позже».",
    3: "На 2 минуты убери телефон. Заметь 3 телесных ощущения и назови их: тепло, напряжение, лёгкость, тяжесть — любые.",
    4: "Перед следующей помощью задай себе один вопрос: «Меня действительно просили об этом?»",
    5: "Назови одно желание на ближайшие 24 часа. Сделай для него самый маленький возможный шаг — максимум 2 минуты.",
}

WORD_OF_DAY = {
    1: ("РАЗРЕШЕНИЕ", "Мне можно быть собой, даже если это не совпадает с чужими ожиданиями."),
    2: ("ГРАНИЦЫ", "Моё «нет» не делает меня плохой."),
    3: ("ВНИМАНИЕ", "Я могу заметить себя раньше, чем доведу себя до предела."),
    4: ("ВЫБОР", "Я не обязана автоматически брать на себя чужое."),
    5: ("АВТОРСТВО", "Моя жизнь состоит не только из обязанностей — в ней есть мои решения."),
}

REFLECTION_CHOICES = {
    "open": "💡 Сделала открытие",
    "calm": "🌿 Стало спокойно",
    "continue": "❤️ Хочу продолжить",
    "think": "📝 Нужно подумать",
}

HARD_HELP = {
    1: "🌿 Тебе не нужно сделать идеально. Если сложно — назови хотя бы одну вещь, рядом с которой тебе становится немного больше собой.",
    2: "💛 Если сложно сказать «нет», начни не с отказа. Начни с паузы: «Я подумаю и вернусь с ответом». Это уже граница.",
    3: "🌿 Не нужно разбираться во всех сигналах тела. Найди один: «сейчас мне тепло / холодно / напряжённо / легко». Этого достаточно.",
    4: "💗 Если сложно перестать спасать — не запрещай себе помогать. Просто добавь вопрос: «Меня просили?»",
    5: "🌸 Если страшно выбирать большой шаг, выбери действие на 2 минуты. Твоя задача — не изменить жизнь сегодня, а показать себе, что ты можешь действовать от себя.",
}

TRACK_FINAL = {
    1: "Ты училась замечать свои опоры, границы, тело и желания.",
    2: "Ты исследовала, где отдаёшь больше энергии, чем получаешь, и где выбираешь «надо» вместо «хочу».",
    3: "Ты возвращала себе право остановиться, слышать тело, замечать чужие сценарии и выбирать себя.",
}

SAVE_CARD_TEXT = {
    1: "Я имею право хотеть того, что важно именно мне.",
    2: "Я имею право не отвечать сразу.",
    3: "Я имею право остановиться и услышать себя.",
    4: "Я имею право помогать только там, где у меня есть ресурс.",
    5: "Я имею право выбрать себя и сделать первый шаг.",
}

# Первое задание теперь одинаковое для всех треков.
FIRST_DAY_TEXT = """☀️ День 1. Мои точки опоры

Сегодня мы не будем искать, что с тобой не так.
Мы начнём с того, что возвращает тебе тебя.

Задание:
• Напиши 5 вещей, занятий, моментов, которые возвращают тебе ощущение «я — это я».
• Напротив каждого пункта напиши, когда в последний раз ты это делала.
• Выбери один пункт и подумай, как вернуть его в свою ближайшую неделю.

Не ищи правильные ответы. Здесь важны именно твои. 🌿"""

# Новое ТЗ: День 1 одинаков для всех треков.
for _track in (1, 2, 3):
    MORNING_TEXTS[(_track, 1)] = FIRST_DAY_TEXT
...
async def show_version_choice_to_chat(context, user_id):
    text = (
        ...
        "💎 дополнительные интерактивные материалы;\n"
...
def schedule_evening_for_day(context, user_id, day, date_value):
    ...
    # Не проверяем наличие .ogg здесь. Проверка должна происходить в момент
    # отправки, иначе отсутствие одного файла полностью убирает 19:00 из расписания.
    schedule_once(
        context,
        evening,
        job_name("evening", user_id, day),
...
# ============================================================
# ИНТЕРАКТИВ ПОСЛЕ ЗАДАНИЙ
# ============================================================

async def send_visual_card(context, user_id: int, day: int):
    path = find_file(f"cards/day{day}.png")
    if not path:
        return
    try:
        with open(path, "rb") as photo:
            await context.bot.send_photo(
                chat_id=user_id,
                photo=photo,
                caption="🌿 Маленькая визуальная опора на сегодня. Сохрани её, если откликается."
            )
    except Exception:
        logger.exception("Ошибка visual card user=%s day=%s", user_id, day)


async def send_save_card(context, user_id: int, day: int):
    path = find_file(f"save_cards/day{day}.png")
    if path:
        try:
            with open(path, "rb") as photo:
                await context.bot.send_photo(
                    chat_id=user_id,
                    photo=photo,
                    caption="🤍 Карточка «Сохрани себе». Вернись к ней, когда понадобится."
                )
            return
        except Exception:
            logger.exception("Ошибка save card user=%s day=%s", user_id, day)
    await context.bot.send_message(chat_id=user_id, text=f"🤍 Сохрани себе:\n\n{SAVE_CARD_TEXT[day]}")


async def send_mini_task(context, user_id: int, day: int):
    text = MINI_TASKS[day]
    markup = InlineKeyboardMarkup([
        [InlineKeyboardButton("✅ Сделала", callback_data=f"mini_done:{day}")],
        [InlineKeyboardButton("⏳ Сделаю позже", callback_data=f"mini_later:{day}")],
        [InlineKeyboardButton("💛 Мне сложно", callback_data=f"hard:{day}")],
    ])
    await context.bot.send_message(
        chat_id=user_id,
        text=f"⏱ Мини-задание на 2 минуты\n\n{text}\n\nЭто не ещё один пункт в списке дел. Это маленькая пауза для контакта с собой.",
        reply_markup=markup,
    )


async def send_task_check_button(context, user_id: int, day: int):
    markup = InlineKeyboardMarkup([
        [InlineKeyboardButton("🌿 Да, сделала", callback_data=f"task_done:{day}")],
        [InlineKeyboardButton("⏳ Сделаю позже", callback_data=f"task_later:{day}")],
        [InlineKeyboardButton("💛 Мне сложно", callback_data=f"hard:{day}")],
    ])
    await context.bot.send_message(
        chat_id=user_id,
        text="Выполнила основное задание?",
        reply_markup=markup,
    )


async def send_evening_reflection(context, user_id: int, day: int):
    markup = InlineKeyboardMarkup([
        [InlineKeyboardButton("💡 Сделала открытие", callback_data=f"reflect:open:{day}")],
        [InlineKeyboardButton("🌿 Стало спокойно", callback_data=f"reflect:calm:{day}")],
        [InlineKeyboardButton("❤️ Хочу продолжить", callback_data=f"reflect:continue:{day}")],
        [InlineKeyboardButton("📝 Нужно подумать", callback_data=f"reflect:think:{day}")],
    ])
    await context.bot.send_message(
        chat_id=user_id,
        text="Что сегодня откликнулось сильнее всего?",
        reply_markup=markup,
    )


async def send_word_of_day(context, user_id: int, day: int):
    word, explanation = WORD_OF_DAY[day]
    await context.bot.send_message(
        chat_id=user_id,
        text=f"✨ Слово дня: {word}\n\n{explanation}"
    )


async def send_musical_pause(context, user_id: int, day: int):
    path = find_file(f"music/day{day}_pause.ogg")
    if path:
        try:
            with open(path, "rb") as audio:
                await context.bot.send_audio(
                    ...
                    performer="Навигатор к себе",
                    caption="🎧 2 минуты, чтобы остановиться. Не делай ничего параллельно."
                )
            return
        except Exception:
            logger.exception("Ошибка музыкальной паузы user=%s day=%s", user_id, day)
    await context.bot.send_message(
        chat_id=user_id,
        text="🎧 Музыкальная пауза\n\nНа 2 минуты включи любую спокойную музыку, которую любишь. Отложи телефон и просто побудь в этом моменте."
    )
...
async def send_morning(context, user_id: int, day: int, scheduled=True):
    ...
    try:
        ...
        # Новые форматы: визуальная карточка + 2-минутная микро-практика.
        await send_visual_card(context, user_id, day)
        await send_mini_task(context, user_id, day)
        await send_task_check_button(context, user_id, day)
...
async def send_evening(context, user_id: int, day: int, scheduled=True):
    ...
    track = int(user["track"] or 0)
    caption = VOICE_CAPTIONS.get((track, day), "🌙 День завершён. Ты справляешься 💖")
    ...
    try:
        ...
        if audio_path and os.path.exists(audio_path):
            ...
            logger.error("Аудиофайл не найден: user=%s track=%s day=%s path=%s", user_id, track, day, audio_path)
            await context.bot.send_message(
                chat_id=user_id,
                text="⚠️ Голосовое сейчас недоступно, но сам вечерний блок продолжается."
            )

        update_user(user_id, last_evening_at=dt_to_db(now_local()))

        # Вечерний интерактив.
        await send_evening_reflection(context, user_id, day)
        await send_word_of_day(context, user_id, day)
        await send_save_card(context, user_id, day)
        if day == 3:
            await send_musical_pause(context, user_id, day)

        await advance_to_next_day(context, user_id, day)

    except Exception:
        logger.exception("Ошибка вечернего блока user=%s day=%s", user_id, day)
        # Даже если один элемент не отправился, не оставляем человека навсегда на одном дне.
        await advance_to_next_day(context, user_id, day)
...
async def send_final_message(context, user_id):
    user = get_user(user_id)
    track = int(user["track"] or 0) if user else 0
    completed = sorted(int(x) for x in json_list(user["completed_days"])) if user else []
    mini_done = sorted(int(x) for x in json_list(user["mini_done_days"])) if user else []
    completed_text = ", ".join(map(str, completed)) if completed else "пока без отметок"
    mini_text = ", ".join(map(str, mini_done)) if mini_done else "пока без отметок"
    ...
    await context.bot.send_message(
        ...
        text=(
            ...
            f"🧭 Твой маршрут:\n{TRACK_FINAL.get(track, '')}\n\n"
            f"📌 Отметки выполненных дней: {completed_text}\n"
            f"⏱ Мини-задания, которые ты отметила: {mini_text}\n\n"
            "Теперь выбери одно открытие, которое хочешь забрать с собой дальше.\n\n"
...
async def send_bonus(context, user_id):
    ...
    completed = sorted(int(x) for x in json_list(user["completed_days"]))
    mini_done = sorted(int(x) for x in json_list(user["mini_done_days"]))
    await context.bot.send_message(
        chat_id=user_id,
        text=(
            "🌺 Ты прошла Челлендж 2.0.\n\n"
            "Спасибо, что прошла этот путь до конца. "
            "Теперь у тебя есть не просто мысли, а конкретные наблюдения и действия.\n\n"
            f"🧭 Твой маршрут: {TRACK_FINAL.get(track, '')}\n\n"
            f"📌 Отметки выполненных дней: {completed or 'пока без отметок'}\n"
            f"⏱ Отмеченные мини-задания: {mini_done or 'пока без отметок'}\n\n"
            "Выбери одно открытие и продолжи с ним уже вне челленджа."
        ),
    )
...
async def send_evening_test(context, user_id: int, day: int):
    user = get_user(user_id)
    if not user or not user["challenge_started"]:
        return
    track = int(user["track"] or 1)
    caption = VOICE_CAPTIONS.get((track, day), "🌙 Тестовое вечернее сообщение")
    await context.bot.send_message(chat_id=user_id, text="🧪 ТЕСТ\n\n" + caption)
    audio_path = AUDIO_FILES.get(track, {}).get(day)
    if audio_path and os.path.exists(audio_path):
        try:
            with open(audio_path, "rb") as audio:
            ...
            logger.exception("Ошибка тестового voice user=%s", user_id)
    else:
        await context.bot.send_message(chat_id=user_id, text="⚠️ Аудиофайл для теста не найден — это не останавливает бота.")
    await send_evening_reflection(context, user_id, day)
    await send_word_of_day(context, user_id, day)
    await send_save_card(context, user_id, day)
...
async def button_handler(update: Update, context):
    ...
    if data.startswith("task_done:") or data.startswith("task_later:"):
        await handle_task_status(update, context)
        return

    if data.startswith("mini_done:") or data.startswith("mini_later:"):
        await handle_mini_status(update, context)
        return

    if data.startswith("reflect:"):
        await handle_reflection_choice(update, context)
        return

    if data.startswith("hard:"):
        await handle_hard(update, context)
        return
...
async def handle_task_status(update, context):
    query = update.callback_query
    await query.answer()
    parts = query.data.split(":")
    day = int(parts[1])
    user = get_user(update.effective_user.id)
    if not user or int(user["current_day"] or 0) != day:
        await query.answer("Эта отметка уже закрыта.", show_alert=True)
        return
    completed = json_list(user["completed_days"])
    if parts[0] == "task_done":
        if day not in completed:
            completed.append(day)
        update_user(user["user_id"], completed_days=dump_list(sorted(set(completed))))
        await query.edit_message_text("🌿 Зафиксировала. Ты сделала это для себя, а не ради галочки.")
    else:
        await query.edit_message_text("⏳ Хорошо. Сделаешь позже — вернись к заданию, когда будет ресурс.")


async def handle_mini_status(update, context):
    query = update.callback_query
    await query.answer()
    parts = query.data.split(":")
    day = int(parts[1])
    ...
    if not user:
        return
    if parts[0] == "mini_done":
        done = json_list(user["mini_done_days"])
        if day not in done:
            done.append(day)
        update_user(user["user_id"], mini_done_days=dump_list(sorted(set(done))))
        await query.edit_message_text("✨ Маленький шаг тоже считается. Зафиксировала твоё участие.")
    else:
        await query.edit_message_text("⏳ Оставь это себе на позже. Тебе не нужно успевать всё сразу.")


async def handle_reflection_choice(update, context):
    query = update.callback_query
    await query.answer()
    _, choice, day_s = query.data.split(":")
    day = int(day_s)
    user = get_user(update.effective_user.id)
    if not user:
        return
    choices = json_list(user["reflection_choices"])
    choices = [x for x in choices if not (isinstance(x, dict) and x.get("day") == day)]
    choices.append({"day": day, "choice": choice})
    update_user(user["user_id"], reflection_choices=json.dumps(choices, ensure_ascii=False))
    await query.edit_message_text(
        f"🌿 {REFLECTION_CHOICES.get(choice, 'Отметка сохранена.')}\n\n"
        "Спасибо, что остановилась на минуту и заметила своё состояние."
    )


async def handle_hard(update, context):
    query = update.callback_query
    await query.answer()
    day = int(query.data.split(":")[1])
    user = get_user(update.effective_user.id)
    if user:
        update_user(user["user_id"], hard_help_count=int(user["hard_help_count"] or 0) + 1)
    await query.message.reply_text(HARD_HELP.get(day, "💛 Не нужно делать идеально. Сделай столько, сколько сейчас можешь."))
...
async def admin_status(update: Update, context):
    ...
    await update.message.reply_text(
        ...
        f"completed_days: {user['completed_days']}\n"
        f"mini_done_days: {user['mini_done_days']}\n"
        f"hard_help_count: {user['hard_help_count']}"
    )
...
async def admin_test_evening(update: Update, context):
    ...
    day = int(user["current_day"] or 1)
    # Для теста не меняем current_day и не завершаем челлендж.
    await send_evening_test(context, user_id, day)
    await update.message.reply_text("✅ Тестовое вечернее сообщение отправлено без изменения прогресса.")
